...
func emit_deployed() -> void:
	deployed.emit()
	print("[TestSignalEmitter] 发出 deployed 信号")

func emit_airborne_changed(is_airborne: bool) -> void:
	airborne_changed.emit(is_airborne)
	print("[TestSignalEmitter] 发出 airborne_changed 信号: %s" % is_airborne)

func emit_action_started(action_type: String) -> void:
	action_started.emit(action_type)
	print("[TestSignalEmitter] 发出 action_started 信号: %s" % action_type)

func emit_action_completed() -> void:
	action_completed.emit()
	print("[TestSignalEmitter] 发出 action_completed 信号")

func emit_hit_received() -> void:
	hit_received.emit()
	print("[TestSignalEmitter] 发出 hit_received 信号")

func emit_destroyed() -> void:
	destroyed.emit()
	print("[TestSignalEmitter] 发出 destroyed 信号")

func emit_disabled_changed(is_disabled: bool) -> void:
	disabled_changed.emit(is_disabled)
	print("[TestSignalEmitter] 发出 disabled_changed 信号: %s" % is_disabled)

func emit_exiting() -> void:
	exiting.emit()
	print("[TestSignalEmitter] 发出 exiting 信号")